  ...
  return <Modal title="确认此语言版的翻译任务" onClose={close}>
    ...
    {p && <><p>复用已封存的来源修订。此语言版拥有独立草稿与当前阅读版本。</p>
      ...
      {p.current_draft_id ? <a className="btn primary" href={`#/drafts/${p.current_draft_id}`}>打开已有内容，继续翻译或编辑</a> : <form onSubmit={e => {e.preventDefault(); void action.run(async () => {
        ...
        <button className="btn primary" disabled={!ready || action.pending || !p.profile_hash || p.profile.dispatch_configuration_ready === false || !p.can_translate || !consent || costControlled && !budgetValid(budget)}>{costControlled ? '确认外发与预算，翻译此语言版' : '确认外发，翻译此语言版'}</button>
      </form>}
    </>}
  </Modal>;
}
