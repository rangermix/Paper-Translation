...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const save = (key, value) => { try { localStorage.setItem('reader-v4:' + key, value); } catch (_) { if (notice) notice.hidden = false; } };
  const read = (key) => { try { return localStorage.getItem('reader-v4:' + key); } catch (_) { if (notice) notice.hidden = false; return null; } };
