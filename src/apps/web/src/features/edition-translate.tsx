...
type EditionPreflight = {preparation_estimates?: PreparationEstimates; profile_hash?: string; generation: number; source_revision_id: string; source_hash: string; source_language: string; locale: string; profile: Provider; current_draft_id?: string; can_translate: boolean; blocked_reason?: string; planned_units?: number; estimated_cost_micro?: number | null};
  ...
  return <Modal title="确认此语言版的翻译任务" onClose={close}>
    ...
    {p && <><p>复用已封存的来源修订。此语言版拥有独立草稿与当前阅读版本。</p>
      ...
      {p.current_draft_id ? <a className="btn primary" href={`#/drafts/${p.current_draft_id}`}>打开已有内容，继续翻译或编辑</a> : <form onSubmit={e => {e.preventDefault(); void action.run(async () => {
        const job = await api<{job_id: string}>(`/editions/${resourceId(editionId)}/translate`, {method: 'POST', etag: etagFor(p), body: {source_revision_id: p.source_revision_id, source_hash: p.source_hash, profile_revision: p.profile.profile_revision, profile_hash: p.profile_hash, ...(costControlled ? { budget_micro: Math.round(Number(budget)*1_000_000) } : {}), external_processing_confirmed: consent, publish_policy: policy, preparation: { mode: preparation }}});
        close(); location.hash = `/jobs/${job.job_id}`;
      }); }}>
        {p.blocked_reason && <p className="notice error-notice">暂不能翻译：{p.blocked_reason}</p>}
        <PreparationControls value={preparation} onChange={mode => { setPreparation(mode); setConsent(false); }} profile={p.profile} estimates={p.preparation_estimates} disabled={action.pending}/><CostControlNotice enabled={costControlled}/>{costControlled && <label className="field">此语言版任务预算（{p.profile.currency ?? 'USD'}）<input className="input" type="number" required min="0.000001" step="0.000001" value={budget} onChange={e => setBudget(e.target.value)}/></label>}
        <label className="field">发布方式<select value={policy} onChange={e => setPolicy(e.target.value as typeof policy)}><option value="manual_approval">手动发布</option><option value="auto_publish">完成后自动发布</option></select></label>
        ...
        <button className="btn primary" disabled={!ready || !preparationAllowed(preparation, p.profile) || action.pending || !p.profile_hash || p.profile.dispatch_configuration_ready === false || !p.can_translate || !consent || costControlled && !budgetValid(budget)}>{costControlled ? '确认外发与预算，翻译此语言版' : '确认外发，翻译此语言版'}</button>
      </form>}
    </>}
  </Modal>;
}
