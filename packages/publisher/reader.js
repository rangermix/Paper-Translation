document.addEventListener('DOMContentLoaded', () => {
  ...
  const save = (key, value) => { try { localStorage.setItem('reader:' + key, value); } catch (_) { if (notice) notice.hidden = false; } };
  const read = (key) => { try { return localStorage.getItem('reader:' + key); } catch (_) { if (notice) notice.hidden = false; return null; } };
