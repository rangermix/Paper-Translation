...
export function ParserSettings({ preferences, loading, onSaved, reload }: {
  ...
  return <section className="panel" aria-label="PDF 解析"><h2>PDF 解析</h2>
    <p className="muted">选择解析方案和可用的运行设备，保存后用于新任务。实际使用的设备显示在任务记录中。</p>
    <div aria-label="检测到的解析环境">
      ...
      {environment.loading ? <Loading/> : detected?.online ? <>
        ...
        <p className="field-note">这是 Docker 解析服务可使用的环境。最近检测：{detected.detected_at ? new Date(detected.detected_at * 1000).toLocaleString() : '未知'}</p>
      </> : <p role="status">解析环境暂不可用或检测结果已过期，请确认解析服务正在运行。</p>}
      <button className="btn" type="button" onClick={environment.reload}>刷新环境</button>
    </div>
    {loading && <Loading/>}
    {preferences && <form onSubmit={e => { e.preventDefault(); if (!validTimeout || !runtimeAvailable || action.pending || action.error) return; void action.run(async () => {
      ...
      <div className="field"><label htmlFor="parser-accelerator">运行设备</label>
        ...
        <div className="field-note" id="parser-accelerator-note">
          {detected?.online && detected.options?.filter(option => !option.profiles.includes(selected)).map(option => <p key={option.id}>{runtimeLabels[option.id]}：{unavailableReasons[option.reason ?? ''] ?? '当前方案不可用。'}</p>)}
          {!runtimeAvailable && <p role="alert">当前选择的设备不可用，请重新选择。</p>}
          {selected === 'paddleocr-vl-1.6-v1' && detected?.memory_bytes && detected.memory_bytes < 10 * 1024 ** 3 ? <p>当前内存较低，Paddle 加载可能因内存不足失败。可增加 Docker 内存或选择较轻的解析方案。</p> : null}
        </div>
      </div>
      ...
      <button className="btn primary" disabled={!validTimeout || !runtimeAvailable || action.pending || Boolean(action.error)}>保存解析设置</button>
    </form>}
    <p className="field-note">识别为公式或代码的区块不参与翻译，并保留原 PDF 裁图。模型识别可能有误，全文覆盖检查仍会执行；已排队或运行中的任务保留原有解析方案、设备与超时设置。超时失败后，请重新发起解析以使用新设置。</p>
  </section>;
}
