...
| 扫描正文/混合PDF | OCR/恢复边界，完整质量认证另计 | 未恢复内容保留原图并提示，不冒充完整译文 |
...
YAML静态结构正确不等于`docker compose config`通过；后者也不等于镜像能够build/up。执行环境没有 Docker CLI 时这两项均标未执行。release gate拒绝空digest/未锁定运行依赖；不能自行填假hash以通过。

## 7. 真实模型验收

M1至少一份已授权的小型en↔zh-Hans样例；原 M2 全阶段认证另需各受控语言样本的人工核对；这不限制产品中所有语言的可用性。仅在派发许可/预算/unknown机制完成后进行；测试预算由维护者显式提供。记录request_id、实际token/金额、不确定费用、输出hash、语义问题。缺少凭据则blocked，不能用演示段落代替。
