...
import type { Capability, Document, Upload, Provider, ParserProfileRevision, PreparationMode } from '../types';
...
export function UploadPage({ capability }: { capability?: Capability }) {
  ...
  async function importRow(row: Row, upload: Upload) {
    ...
    try { const doc = await api<Document>('/imports', { method: 'POST', body: { source: { kind: 'pdf_upload', upload_id: upload.upload_id }, source_language: 'auto', target_language: language, parser_profile_revision: selectedParser, workflow: { translate, preparation: { mode: preparation }, target_locale: language, publish_policy: policy, external_processing_confirmed: translate && consent, profile_hash: profile?.profile_hash, ...(costControlled && budgetValid(budget) ? { budget_micro: Math.round(Number(budget) * 1_000_000) } : {}) } } }); update(row.id, { document: doc, busy: false }); }
...
{row.document && <p role="status"><a href={`#/documents/${row.document.id}`}>已入库，后台处理中 · 查看文档</a></p>}{row.upload?.duplicate_documents?.length && !row.document ? <div className="notice"><strong>发现相同 PDF 内容</strong><p>可继续使用已有文档，或建立独立目录条目；不会按文件名覆盖旧版本。</p>{row.upload.duplicate_documents.map(doc => <a className="btn sm" key={doc.id} href={`#/documents/${doc.id}`}>使用已有文档：{doc.title}</a>)}<button className="btn sm" disabled={!ready || row.busy || translate && !preparationAllowed(preparation, profile) || !isLanguageTag(language)} onClick={() => void importRow(row, row.upload!)}>建立独立文档</button></div> : null}</div>)}<div className="form-actions"><span className="muted small">有效性、加密状态和页数以服务端检查为准。</span><button className="btn primary" disabled={!ready || busy || translate && !preparationAllowed(preparation, profile) || !isLanguageTag(language) || !rows.some(r => !r.error && !r.upload && !r.document)} onClick={() => void uploadAll()}>{busy ? '正在接收与检查…' : '上传并开始处理'}</button></div></section><aside className="panel"><h2>处理方式</h2>
      ...
      {translate && <><ErrorNotice error={provider.error} retry={provider.reload}/><ProviderDestination profile={profile}/>
        <p className="field-note">翻译模型：{profile?.model_id || '尚未配置'}</p><PreparationControls value={preparation} onChange={mode => { setPreparation(mode); setConsentHash(''); }} profile={profile} disabled={busy}/><CostControlNotice enabled={costControlled}/>
