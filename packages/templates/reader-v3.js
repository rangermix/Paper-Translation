document.addEventListener('DOMContentLoaded', () => {
  ...
  const save = (key, value) => { try { localStorage.setItem('reader-v3:' + key, value); } catch (_) { if (notice) notice.hidden = false; } };
  const read = (key) => { try { return localStorage.getItem('reader-v3:' + key); } catch (_) { if (notice) notice.hidden = false; return null; } };
  const theme = read('theme');
  if (theme === 'dark' || theme === 'light') root.dataset.theme = theme;
  // The frozen styles use a fixed anchor margin, but the toolbar can wrap.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelector('[data-action="target"]')?.click();
});
