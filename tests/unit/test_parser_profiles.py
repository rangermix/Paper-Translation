...
from packages.parsers.profiles import DEFAULT_PROFILE, DOCLING_PROFILE, GRANITE_PROFILE, PADDLE_PROFILE, selected_profile
...
def test_profile_validation_and_legacy_default():
    assert selected_profile({}) == DOCLING_PROFILE == 'docling-v1'
...
def test_selected_pipeline_changes_fingerprint():
    ...
    assert pipeline_fingerprint(lock, DOCLING_PROFILE) != pipeline_fingerprint(lock, GRANITE_PROFILE)
...
def test_spool_accepts_only_supported_profiles():
    ...
    for profile in (DOCLING_PROFILE, DOCLING_PROFILE, GRANITE_PROFILE, PADDLE_PROFILE):
