...
- `compose.example.yaml` 是唯一受版本管理的 Compose 模板；CPU 默认启用，CUDA/MLX 在复制后的文件中选择。根目录 `compose.yaml` 是忽略的本地配置，保留用户的项目名、镜像绑定、MLX 设置和本地覆盖文件；维护现有实例前检查实际目标。测试/检查显式使用模板和独立项目，不能用 profile 整体 `up` 启动测试或用测试清理操作用户实例。
...
- 翻译服务通过应用设置管理；API key 只持久化到 `provider_config` 卷中的后端 secret 文件，不回显，不写浏览器存储、解析器、日志或导出，不新增 provider profile/key 宿主绑定。保存设置不自动测试。目的地、协议、鉴权、model ID 与已确认配置必须一致，不自动切换供应商或模型。
